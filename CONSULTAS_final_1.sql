...
SELECT 
    ROUND(MIN(replacement_cost), 2) AS minimo,
    ROUND(MAX(replacement_cost), 2) AS maximo,
...
SELECT 
    ROUND(AVG(amount), 2) AS promedio_total,
    ROUND(STDDEV(amount), 2) AS desviacion_estandar,
    ROUND(VAR_SAMP(amount), 2) AS varianza_muestral
FROM payment;
...
WHERE r.rental_id IS NULL  -- Solo películas que NO están en alquiler
...
WITH conteo_nombres AS (
    SELECT first_name AS nombre,
           COUNT(*) AS total,
           MAX(COUNT(*)) OVER () AS max_repeticiones
    FROM actor
    GROUP BY first_name
)
SELECT nombre, total,
       CASE WHEN total = max_repeticiones THEN 'MÁS REPETIDO' ELSE '' END AS estado
FROM conteo_nombres
